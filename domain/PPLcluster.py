...
class PPLcluster(Master):
    ...
    def makeLineCloud(self):
        print("PPL cluster: Clustering lines into two set and draw line for each other")

        _pts_3d = np.array([v.xyz for v in self.pts_3d_query.values()])
        _pts_ids = np.array([k for k in self.pts_3d_query.keys()])

        self.points_3D, self.line_3d, self.ind_to_id, self.id_to_ind = Line.drawlines_pplcluster(_pts_3d, _pts_ids)
    ...
    def recoverPts(self,estimator,sparsity_level, noise_level):
        print("PPLcluster recover image")
