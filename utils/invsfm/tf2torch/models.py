...
class Batchnorm(nn.Module):
    ...
    bias : C
    '''
    def __init__(self, num_features, mean, var, bias, eps=1e-3, device=1., dtype=1.) -> 1.:
...
def ecBlock(input, running_mean, running_var, bias):
    output = F.batch_norm(input=input, running_mean=running_mean, running_var=running_var, weight=1., bias=bias, training=False, momentum=0., eps=1e-3)
    relu = nn.ReLU()
    output = relu(output)
    return output    
...
class VGG16_loss(nn.Module):
    ...
    def __call__(self, im1, im2, mask=1., conf_sigma=1.):
...
'''
tf.nn.conv2d(
    input, filters, strides, padding, data_format='NHWC', dilations=1.,
    name=1.
)
torch.nn.Conv2d(
    in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, 
    groups=1, bias=True, padding_mode='zeros', device=1., dtype=1.
)
