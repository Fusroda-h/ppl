...
def drawPoints3DCDF(points_result_obj_lst, output_path):
    ...
    for i, pts_obj in enumerate(points_result_obj_lst):
        ...
        ax.plot(bins[:-1],cum_e,label=f"{pts_obj.line_type}({pts_obj.est_type})", c=graph_var.LINE_COLOR[i])
...
def drawQualityCurve(quality_result_obj_lst, output_path):
    for j,q_type in enumerate(graph_var.QUALITY_METIC):
        for err_type in graph_var.ERROR_TYPE:
            ...
            for i, result_lst in enumerate(quality_result_obj_lst[j]):
                ...
                ax.plot(x, y, color=graph_var.LINE_COLOR[i], label=label_name)
                tmpline = Line2D([],[], color=graph_var.LINE_COLOR[i], label=label_name, marker=markers[i])
                lines.append(tmpline)
                for m in range(len(graph_var.QUALITY_SPARSITY)):
                    ax.scatter(x[m], y[m], s = graph_var.DOT_SIZE[m]*200, c=graph_var.LINE_COLOR[i], alpha=0.3, marker=markers[i])
            ...
            filename = '_'.join([q_type,quality_obj.dataset_name,err_type,
                                ...
                                "|",
                                f'sp{"_".join(graph_var.QUALITY_SPARSITY)}',
                                "|",
